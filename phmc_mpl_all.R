...
phmc_mpl.control=function(basis="msplines",maxIter=c(150,75e+02,1e+04),epsilon=c(1e-16, 1e-10),kappa=1/0.6,conv_limit=1e-4,
  ...
  #basis is the name of the basis function being used i.e. msplines
  #smooth is the smoothing parameter - by default set to null and estimated by REML, MLE obtained by setting smooth = 0, needs to be non-negative (can be specified to a fixed number)
  #max.iter is a vector defining the max number of iterations for the smooth parameter (first value), for estimating beta and theta, third value is total number of iterations allowed
  #tol is the convergence tolerance value (convergence achieved with abs difference is smaller than tol)
  #n.knots is vector of two integers defining the internal knot sequence of non-uniform basis should be set
  #range.quant vector of length 2 defining the range of the quantile knots when a non-uniform basis is chosen
  #min.theta a value indicating the minimum baseline hazard parameter in the output - anything smaller than this will be considered zero
  #penalty integer specifying the order of the penalty matrix
  #order integer specifying the order of the msplines basis
  #kappa value larger than 1 used in fitting algorithm to decrease the step size when penalised likelihood doesnt decrease during iterative process
  #epsilon a vector of 2 values indicating the minimum distrance from 1 and from 0 of the survival function and the baseline parameter estimates (to avoid issues with logarithms/dividing by 0)
  #ties a character string indicating a method to handle duplicated outcomes when defining the knots sequence
  #seed null or an integer vector to get random noise number to fix ties if using epsilon method 
...
phmc_mpl = function(ph.formula,pi.formula,data,control,...){
  ...
  for(iter in 1:control$maxIter[1]){
    ...
    #if((full.iter/iter)>(maxIter[2]*.975)){epsilon[2] = epsilon[2]*10}
